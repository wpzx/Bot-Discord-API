...
async function getWhitelistFromSheets() {
  try {
    const response = await sheets.spreadsheets.values.get({
      ...
      range: `${SHEET_NAME}!A2:E`, // Mulai dari baris 2 (skip header)
    })
...
async function saveWhitelistToSheets(servers) {
  try {
    ...
    // Prepare rows
    const rows = servers.map((server) => [server.ip, server.owner, server.addedBy, server.addedAt, server.status])
 ...
 * Cek apakah IP ada di whitelist (untuk gamemode)
 */
 ...
 * Menambah server ke whitelist (untuk bot Discord)
 */
 ...
 * Menghapus server dari whitelist (untuk bot Discord)
 */
